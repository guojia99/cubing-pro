...
type CompResp struct {
	...
	Group competition.CompertionGroup `json:"Group"`
...
func Comp(svc *svc.Svc) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		var group competition.CompertionGroup
		...
		svc.DB.Model(&competition.Competition{}).Where("created_at < ?", comp.CreatedAt).Select("id").
			Order("created_at DESC").Limit(1).Pluck("id", &earliestID)
		svc.DB.Model(&competition.Competition{}).Where("created_at < ?", comp.CreatedAt).Select("name").
