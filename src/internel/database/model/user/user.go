...
type User struct {
	...
	WcaLoginAt        *time.Time `gorm:"column:wca_login_at;null" json:"WcaLoginAt,omitempty"`    // 最近一次 WCA 登录时间
	WcaAccessToken    string     `gorm:"column:wca_access_token;null" json:"-"`                   // WCA OAuth access token 缓存
	WcaTokenExpiresAt *time.Time `gorm:"column:wca_token_expires_at;null" json:"-"`               // WCA token 过期时间
