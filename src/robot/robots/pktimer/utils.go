...
func (p *PkTimer) getIniterMessage(results *pktimerDB.PkTimerResult) string {
...
func (p *PkTimer) initPkTimer(msg types.InMessage) error {
	...
	p.sendMessage(msg.NewOutMessage(p.getIniterMessage(&newPKTimerResult)))
	return nil
}
...
func (p *PkTimer) in(msg types.InMessage) error {
	...
	for _, pl := range pkTimerResult.PkResults.Players {
		if pl.UserId == usr.ID {
			...
			p.sendMessage(msg.NewOutMessage(p.getIniterMessage(pkTimerResult)))
			return nil
		}
	}
	...
	for idx, pl := range pkTimerResult.PkResults.Players {
		if pl.UserId == usr.ID {
			...
			if len(pkTimerResult.PkResults.Players[idx].Results) == pkTimerResult.PkResults.CurCount {
				...
				p.sendMessage(msg.NewOutMessagef("pk 成绩更新成功: %s | (%d/%d)", result.TimeParserF2S(res), len(pkTimerResult.PkResults.Players[idx].Results), pkTimerResult.PkResults.CurCount))
				break
			}
			pkTimerResult.PkResults.Players[idx].Results = append(pkTimerResult.PkResults.Players[idx].Results, res)
			p.sendMessage(msg.NewOutMessagef("pk 成绩记录成功: %s | (%d/%d)", result.TimeParserF2S(res), len(pkTimerResult.PkResults.Players[idx].Results), pkTimerResult.PkResults.CurCount))
			break
		}
	}
	p.Svc.DB.Save(&pkTimerResult)
